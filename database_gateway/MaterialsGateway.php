...
//
    public function select_all_materials() {
        $materials_rec = mysql_query("SELECT * FROM " . $this->get_material_table_name() . " ORDER BY Add_Date ASC;")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');

        $materials = array();
        while (($obj = mysql_fetch_object($materials_rec)) != NULL) {
            $sections = $this->section_gateway->select_section($obj->section_id);
            $section = $sections[0];
...
//
    public function select_materials_by_section($section) {
        ...
        $materials_rec = mysql_query($query)or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
//
    public function select_material($file_name, $section_id) {
        ...
        $materials_rec = mysql_query("SELECT * FROM " . $this->get_material_table_name() . " where section_id =$section_id and file_name='$file_name';")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
//
    ...
    private function deleteFile($file_name, $section_id) {
        $sections = $this->section_gateway->select_section($section_id);
        $section = $sections[0];
